...
**Трассируемость.** Каждый агент фиксирует свой `output_data` в `memory/TASK_MEMORY_*.yml`. Это создаёт полный audit trail: кто что решил, на каком этапе и почему.
...
Поднимаются Vite (клиент) и dev-сервер парсера/стриминга. Порт API по умолчанию задаётся переменной `PORT` (если не указана — **3001**). Бэкенд по умолчанию читает состояние из **`<корень_репозитория>/memory`**; при другом расположении укажите абсолютный путь к каталогу с доской и YAML в `DASHBOARD_DATA_DIR` или `DEV_STUDIO_DATA_DIR`. Подробности по стеку и линтингу — в [`dashboard/README.md`](dashboard/README.md).
