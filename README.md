...
Мультиагентная система оркестрации для Cursor: в едином координированном процессе (SDLC) участвуют роли:
...
Каждый запуск агента — отдельная запись `Execution` в памяти задачи. Статус меняется линейно. **С точки зрения субагента:** к моменту его старта в памяти уже есть его execution в статусе `new` (его добавляет оркестратор до вызова субагента); исполнение задачи начинается только после того, как субагент перевёл этот execution в `in-progress` и сохранил YAML.

```mermaid
stateDiagram-v2
    direction LR
    [*] --> new : оркестратор добавляет execution (new), затем запускает субагента
    new --> in_progress : субагент фиксирует старт этапа
